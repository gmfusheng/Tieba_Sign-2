...
class plugin_zw_mailauth extends Plugin {
  ...
  PRIMARY KEY (`id`)
	...
	function before_register() {
		...
		$authlink = "<a href=".$authlink." target=\'_blank\'>".$authlink."</a>";
		// 检查是否有特定标记并进行替换
	...
	function on_load() {
		switch ($_GET['action']) {
			case 'verify':
				...
				if (!$authinfo) {
					showmessage("没有找到待验证记录，或验证已通过<br>如有问题请联系站长QQ：<a target='_blank' href='http://sighttp.qq.com/authd?IDKEY=b7bd7716a17ebd869f4c267c7ec6e8e1dab69f790186f176'>910323144</a>", dreferer(), 3);
					...
					if ($nowtime > $authinfo['regtime'] + $setting['deathtime'] * 60) {
						...
						showmessage("恭喜您，验证成功~~~<br>您的用户名是：{$username}<br>现在赶快登录吧~~~", dreferer(), 3);
					} 
				} else {
